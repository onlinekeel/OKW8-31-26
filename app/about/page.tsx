    ...
    <>
      ...
      <section className="section-space bg-white"><div className="site-shell grid gap-12 lg:grid-cols-[0.72fr_1.28fr]">
        ...
        <div className="grid gap-0">
          {[["01", "Be direct", "Plain language, honest recommendations, and no unnecessary complexity."], ["02", "Respect ownership", "Clients keep control of the domain, hosting, accounts, and finished work."], ["03", "Design responsibly", "Commercial-safe fonts and graphics."], ["04", "Make it last", "A flexible system that can grow without requiring a rebuild for every new chapter."]].map(([number, title, copy]) => <article key={number} className="grid gap-3 border-t border-[#0b2530]/12 py-7 sm:grid-cols-[3rem_0.65fr_1.35fr] sm:gap-7"><span className="font-serif text-lg text-[#b08f54]">{number}</span><h3 className="font-serif text-2xl text-[#0b2530]">{title}</h3><p className="text-sm leading-6 text-[#64767b]">{copy}</p></article>)}
        </div>
      </div></section>
      <CtaBand />
    </>
