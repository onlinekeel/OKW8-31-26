...
export default function ServicesPage() {
  return (
    <>
      ...
      <section className="pb-20 sm:pb-32"><div className="site-shell grid gap-5">
        {offers.map((offer) => (
          <article id={offer.id} key={offer.id} className="scroll-mt-24 rounded-[1.5rem] border border-[#0b2530]/12 bg-white p-6 sm:scroll-mt-28 sm:rounded-[2rem] sm:p-10 lg:p-12">
            <div className="grid gap-7 sm:gap-9 lg:grid-cols-[0.62fr_1.05fr_0.9fr]">
              ...
              <div><h2 className="font-serif text-[2.35rem] leading-[1.05] tracking-[-0.035em] text-[#0b2530] sm:text-5xl">{offer.title}</h2><p className="mt-5 max-w-xl text-base leading-7 text-[#617278] sm:mt-6">{offer.copy}</p><Link href="/contact" className="text-link mt-6 sm:mt-7">Talk about this package <ArrowRight className="size-4" /></Link></div>
              <ul className="grid content-start gap-3 border-t border-[#0b2530]/10 pt-6 lg:border-l lg:border-t-0 lg:pl-9 lg:pt-0">{offer.items.map((item) => <li key={item} className="flex gap-3 text-base leading-7 text-[#405860]"><Check className="mt-1 size-4 shrink-0 text-[#6b9a91]" /> {item}</li>)}</ul>
            </div>
          </article>
        ))}
      </div></section>
      ...
      <section className="section-space bg-white"><div className="site-shell grid gap-10 lg:grid-cols-[0.8fr_1.2fr]">
        ...
        <div className="rounded-[1.5rem] bg-[#eef1ec] p-6 sm:rounded-[2rem] sm:p-10"><p className="font-serif text-[2.25rem] leading-tight text-[#0b2530] sm:text-4xl">As-needed support</p><p className="mt-5 max-w-2xl text-base leading-7 text-[#5d7076]">Post-launch content changes and small improvements are available when you need them. We outline the work and confirm the cost before anything begins, so you only approve support you actually want.</p><p className="mt-5 text-base leading-7 text-[#5d7076]">Hosting and domain costs are paid directly by you, so your site never depends on a proprietary maintenance plan to stay online.</p></div>
      </div></section>
      <CtaBand />
    </>
  );
