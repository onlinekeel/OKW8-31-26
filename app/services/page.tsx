  ...
  return (
    <>
        ...
        {offers.map((offer) => (
          <article id={offer.id} key={offer.id} className="scroll-mt-28 rounded-[2rem] border border-[#0b2530]/12 bg-white p-7 sm:p-10 lg:p-12">
            <div className="grid gap-9 lg:grid-cols-[0.62fr_1.05fr_0.9fr]">
              ...
              <div><h2 className="font-serif text-4xl tracking-[-0.035em] text-[#0b2530] sm:text-5xl">{offer.title}</h2><p className="mt-6 max-w-xl text-base leading-7 text-[#617278]">{offer.copy}</p><Link href="/contact" className="text-link mt-7">Talk about this package <ArrowRight className="size-4" /></Link></div>
      ...
      <section className="section-space bg-white"><div className="site-shell grid gap-10 lg:grid-cols-[0.8fr_1.2fr]">
        ...
        <div className="rounded-[2rem] bg-[#eef1ec] p-7 sm:p-10"><p className="font-serif text-4xl text-[#0b2530]">As-needed support</p><p className="mt-5 max-w-2xl text-base leading-7 text-[#5d7076]">Post-launch content changes and small improvements are available when you need them. We outline the work and confirm the cost before anything begins, so you only approve support you actually want.</p><p className="mt-5 text-sm leading-6 text-[#5d7076]">Hosting and domain costs are paid directly by you, so your site never depends on a proprietary maintenance plan to stay online.</p></div>
      </div></section>
      <CtaBand />
    </>
  );
