...
export default function ProcessPage() {
  return (
    <>
      ...
      <section className="pb-20 sm:pb-32"><div className="site-shell">
        {phases.map((phase) => (
          <article key={phase.number} className="grid grid-cols-[2.5rem_1fr] gap-x-4 gap-y-5 border-t border-[#0b2530]/13 py-8 lg:grid-cols-[0.28fr_0.7fr_1.1fr_0.8fr] lg:gap-10 lg:py-12">
            ...
            <p className="col-span-2 max-w-2xl text-base leading-7 text-[#566b72] lg:col-span-1">{phase.copy}</p>
            <div className="col-span-2 lg:col-span-1"><p className="text-xs font-semibold uppercase tracking-[0.15em] text-[#79908f]">You can expect</p><p className="mt-3 text-base leading-7 text-[#3f5860]">{phase.deliverable}</p></div>
          </article>
        ))}
      </div></section>
      <section className="section-space bg-[#eef1ec]"><div className="site-shell grid gap-12 lg:grid-cols-[0.78fr_1.22fr]">
        ...
        <div className="grid gap-5 sm:grid-cols-2">
          {[["Your vision", "Clear ownership of your vision keeps feedback coherent and decisions timely."], ["Real content early", "The sooner we work with your actual words and images, the better the design becomes."], ["Focused feedback", "Specific reactions tied to the project goals produce stronger refinements."], ["No mystery handoff", "You receive direct account access and have full ownership of the site with instructions on how to manage it."]].map(([title, copy]) => <article key={title} className="rounded-3xl bg-white p-6 sm:p-7"><h3 className="font-serif text-2xl text-[#0b2530]">{title}</h3><p className="mt-3 text-base leading-7 text-[#64767b]">{copy}</p></article>)}
        </div>
      </div></section>
      <CtaBand />
    </>
